...
for i in range(0,rows,1):
        if(this_class.get(i)!= None):
            ...
            int_error+=(-this_class[i]*math.log(sig))-((1-this_class[i])*math.log(1-sig))
            #int_error+=(this_class[i]-math.log(1+math.exp(-1*dp)))**2
    ...
    for i in range (0,rows,1):
        if(this_class.get(i)!= None):
            ...
            for j in range(0,cols,1):
                dp+=w[j]*float(data[i][j])
            for j in range(0,cols,1):
                dellf[j]+=(-this_class[i]+(1.0/(1+math.exp(-dp))))*float(data[i][j])
        #if ends here
    #print(dellf)
    for j in range(0,cols,1):
        w[j]=w[j]-eta*dellf[j]
    ...
    for i in range(0,rows,1):
        if(this_class.get(i)!= None):
            ...
            for j in range(0,cols,1):
                dp+=w[j]*float(data[i][j])
            sig=(1/(1+math.exp(-1*dp)))
            error+=(-this_class[i]*math.log(sig))-((1-this_class[i])*math.log(1-sig))
            #error+=(this_class[i]-math.log(1+math.exp(-1*dp)))**2
        #if ends here
    if abs(int_error - error) <= stp:
        converged = True 
    #print(abs(int_error - error))
    int_error = error
    #print(int_error - error)
